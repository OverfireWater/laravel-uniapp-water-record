                ...
                <view class="popup-bg">
                    <view class="popup-content" :class="{'popup-content-show' : popup_show}">
                        <view class="update-wrap">
                            <image src="../../components/app-update/images/img.png" class="top-img"></image>
                            <view class="content">
                                <text class="title">发现新版本V{{update_info.version}}</text>
                                <!-- 升级描述 -->
                                <view class="title-sub" v-html="note"></view>
                                <!-- 升级按钮 -->
                        ...
                        <image src="../../components/app-update/images/close.png" class="close-ioc" @click="closeUpdate()" v-if="downstatus < 1 && update_info.force == 0"></image>
                    </view>
                </view>
            </view>
        </view>
    ...
    export default {
        ...
        onLoad() {
            if (this.appInfo.version) {
                this.update_info = this.appInfo
                this.note =JSON.parse(this.update_info.note); //版本说明
            } else {
        ...
        methods: {
            ...
            startUpdate() {
                ...
                if (/\.apk$/.test(this.update_info.update_url)) {
                    // 如果是apk地址
                    this.download_wgt() // 安装包/升级包更新
                } else if (/\.wgt$/.test(this.update_info.update_url)) {
                    // 如果是更新包
                    this.download_wgt() // 安装包/升级包更新
                } else {
                    plus.runtime.openURL(this.update_info.update_url, function() { //调用外部浏览器打开更新地址
            ...
            download_wgt() {
                ...
                let dtask = plus.downloader.createDownload(this.update_info.update_url, options);
    ...
    .sche-wrap {
        ...
        .sche-bg {
            ...
            .sche-bg-jindu {
                ...
                min-width: 40rpx;
                border-radius: 100px;
                background: url(../../components/app-update/images/round.png) #5775e7 center right 4rpx no-repeat;
